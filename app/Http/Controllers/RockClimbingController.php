...
class RockClimbingController extends Controller
{
    ...
    {
        // Get rock climbing destinations from AdventureDestinationService
        $climbingData = AdventureDestinationService::getDestinationsByCategory('rock-climbing');
        
        // Get weather data for each destination
        $destinationsWithWeather = [];
        $destinations = $climbingData['destinations'] ?? [];
        foreach ($destinations as $destination) {
            $city = $destination['location'] ?? 'Kathmandu'; // Default to Kathmandu if location not specified
            
            // Extract city name from location string (e.g., "Nagarkot, Nepal" -> "Nagarkot")
        ...
        return view('pages.rock-climbing', [
            'climbingData' => $destinationsWithWeather,
            'pageTitle' => 'Rock Climbing Adventures in Nepal',
            'metaDescription' => 'Challenge yourself with rock climbing in Nepal\'s stunning locations. From beginner-friendly routes to technical climbs with Himalayan views.'
        ]);
    }
