...
class TrekkingController extends Controller
{
    ...
    {
        // Get trekking destinations from AdventureDestinationService
        $trekkingData = AdventureDestinationService::getDestinationsByCategory('trekking');
        
        // Get weather data for each destination
        $destinationsWithWeather = [];
        $destinations = $trekkingData['destinations'] ?? [];
        foreach ($destinations as $destination) {
            $city = $destination['location'] ?? 'Kathmandu'; // Default to Kathmandu if location not specified
            
            // Extract city name from location string (e.g., "Everest Base Camp, Nepal" -> "Everest Base Camp")
        ...
        return view('pages.trekking', [
            'trekkingData' => $destinationsWithWeather,
            'pageTitle' => 'Himalayan Trekking Adventures',
            'metaDescription' => 'Discover the majestic Himalayas through our guided trekking adventures. From Everest Base Camp to Annapurna Circuit, experience Nepal\'s most iconic trails.'
        ]);
    }
